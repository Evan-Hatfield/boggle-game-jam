...
func _ready():
	...
	var file = FileAccess.open("res://BoggleDict.txt", FileAccess.READ)
	var content = file.get_as_text().split("\r\n")

	for word in content:
		insertWord(root, word)
	PlayerVars.treeroot = root
...
func _process(delta):
	if Input.is_action_just_pressed("enter"):
		...
		if (goodWord): 
			...
			if (!(PlayerVars.alliterator_checker == alliteratorWord) and PlayerVars.relicsowned[0]):
				relicPasser = false
			if (len(wordin) < 5 and PlayerVars.relicsowned[2]):
				relicPasser = false
	...
	if(time_since_enabled > 2):
		...
		#var wordin = $LineEdit.text 
		#if querylist(wordin):
			#if (wordin not in correctWordsList):
				#sound_player.stream = sound_effect
				#sound_player.play()
				#$found.visible = true
				#time_since_enabled = 0
